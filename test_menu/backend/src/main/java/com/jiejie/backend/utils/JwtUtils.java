package com.jiejie.backend.utils;
...
public class JwtUtils {
    ...
    public DecodedJWT resolveJwt(String headerToken){
        String convert = this.convertToken(headerToken);
        if (convert == null) {
            return null;
        }
        Algorithm algorithm = Algorithm.HMAC256(key);
        ...
        try{
            DecodedJWT verify = jwtVerifier.verify(convert);
    ...
    private boolean frequencyCheck(int userId){
        String key = Constant.JWT_FREQUENCY + userId;
